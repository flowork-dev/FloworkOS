...
# File NAME : C:\FLOWORK\ai_providers\text\chatgpt_provider\provider.py total lines 69 
########################################################################
...
class ChatGPTProvider(BaseAIProvider):
    ...
    def _stream_response(self, prompt, messages=None):
        ...
        if hasattr(self.connection, 'get_chat_stream'):
            ...
            yield " [System: Streaming not yet fully implemented in Connection class] "
            full_res = self.connection.get_chat_completion(prompt, messages=messages)
            yield full_res.get("data", "")
            return
