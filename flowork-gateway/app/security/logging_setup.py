...
# File NAME : C:\FLOWORK\flowork-gateway\app\security\logging_setup.py total lines 125 
########################################################################
...
_PATTERNS = [
    ...
    re.compile(r'([A-Fa-f0-9]{64,})'), # Long Hex (secrets)
    re.compile(r'([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})'), # Email
    re.compile(r'(fk-admin-[a-zA-Z0-9_-]{40,})'), # Flowork Admin Token
    re.compile(r'(fk-eng-[a-zA-Z0-9_-]{60,})'), # Flowork Engine Token
]

class RedactionFilter(logging.Filter):
    """
    (English Hardcode) Filters log records to redact sensitive patterns
    (English Hardcode) and remove full 'fac' objects from extras.
    """
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if hasattr(record, "fac") and record.fac:
            ...
            del record.fac # (English Hardcode) Remove the dangerous original

        return True

class JsonFormatter(logging.Formatter):
    """
    (English Hardcode) Formats log records as a single-line JSON object.
    """
    def format(self, record: logging.LogRecord) -> str:
        obj = {
            "ts": round(record.created, 3), # (English Hardcode) Use record's creation time
            "level": record.levelname,
            "logger": record.name,
            "msg": record.getMessage(), # (English Hardcode) Get final (redacted) message
        }
...
def configure_logging() -> None:
    """
    (English Hardcode) R9.2: Configure the root logger with JSON/Redaction.
    (English Hardcode) This should be called ONCE at startup.
    """
...
def fac_summary(fac: Dict[str, Any]) -> Dict[str, Any]:
    """
    (English Hardcode) R9.2: Produce a minimal FAC summary safe for logs.
    (English Hardcode) Avoids leaking capabilities/signature.
    """
