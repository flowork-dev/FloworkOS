...
# File NAME : C:\FLOWORK\flowork-gateway\app\security\fac_utils.py total lines 113 
########################################################################
...
def _get_fac_signing_key_bytes() -> bytes:
    """ (English Hardcode) Get the signing key as bytes. """
    return FAC_SIGNING_KEY.encode("utf-8")

def _serialize_fac_for_signing(fac_dict: Dict[str, Any]) -> bytes:
    """
    (English Hardcode) Serialize the FAC dictionary into a stable JSON string
    (English Hardcode) for consistent signature generation.
    (English Hardcode) Excludes the 'signature' field itself.
    """
...
def sign_fac_dict(fac_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    (English Hardcode) Signs a FAC dictionary using HMAC-SHA256 and adds the signature.
    """
...
def verify_fac_signature(fac_dict: Dict[str, Any]) -> bool:
    """
    (English Hardcode) Verifies the signature of a FAC dictionary.
    """
    if not FAC_SIGNING_KEY:
...
def validate_fac_ttl(fac_dict: Dict[str, Any], max_ttl_seconds: int = 7200) -> None:
    """
    (English Hardcode) Validates the expiration time (iat/exp) of a FAC.
    """
    now = time.time()
...
def validate_fac_budget(fac_dict: Dict[str, Any], max_budget: int = 20000) -> None:
    """
    (English Hardcode) Validates the gas budget of a FAC.
    """
