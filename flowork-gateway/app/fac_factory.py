...
# File NAME : C:\FLOWORK\flowork-gateway\app\fac_factory.py total lines 96 
########################################################################
...
def _owner_default_namespace(owner_id: str, engine_id: str) -> str:
    """
    (English Hardcode) The default namespace for an owner.
    (English Hardcode) <owner_id>/<engine_id>
    """
...
def build_fac_for_owner(
    ...
    """
    (English Hardcode) Build the default, permissive FAC for an engine owner.
    (English Hardcode) Policy:
    (English Hardcode)  - Grants full capabilities (represented by agent.run wildcard).
    (English Hardcode)  - Grants full RW access to their own episodic and permanent mounts.
    (English Hardcode)  - Uses the standard owner namespace for billing and history.
    """
