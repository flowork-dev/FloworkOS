...
# File NAME : C:\FLOWORK\flowork-gateway\app\sockets.py total lines 1372 
########################################################################
...
from .models import User, RegisteredEngine, UserEngineSession, EngineShare, AgentSession, Episode
...
def on_forward_event_to_gui(data):
    """
    (GEMINI ADDED - Based on User Analysis)
    This is the crucial handler that was missing.
    It receives events from the Core (like workflow status updates)
    and forwards them to the correct user in the GUI namespace.
    """
    app = current_app._get_current_object()
    ...
    app.logger.info(f"[Gateway] Forwarding Engine event '{event_name}' to GUI room {user_id}")
    ...
    app.logger.info(f"[Gateway] Successfully emitted '{event_name}' to GUI room {user_id}")
...
def on_request_presets_list(data):
    ...
    try:
        ...
        if not eng_id or not eng_sid:
            ...
            app.logger.warning(f"[Gateway] Cannot forward 'request_presets_list'. No LIVE SID for engine {target_engine_id or 'default active'}. UserID: {user_id}")
...
def on_request_variables(data):
    ...
    try:
        ...
        if not eng_id or not eng_sid:
            ...
            app.logger.warning(f"[Gateway] Cannot forward 'request_variables'. No LIVE SID for engine {target_engine_id or 'default active'}. UserID: {user_id}")
...
def on_request_components_list(data):
    ...
    try:
        ...
        if not eng_id or not eng_sid:
            ...
            if not user_id:
                err_msg = 'Cannot fetch components. Please log in to connect to an engine.'
            app.logger.warning(f"[Gateway] Cannot forward 'request_components_list' for '{comp_type}'. No LIVE SID for engine {target_engine_id or 'default active'}. UserID: {user_id}")
...
def on_request_ai_status(data):
    ...
    try:
        ...
        if not eng_id or not eng_sid:
            err_msg = f'Engine {target_engine_id or "active"} is not connected to the Gateway.'
            if not user_id:
                err_msg = 'Cannot fetch AI status. Please log in to connect to an engine.'
            sio.emit('response_ai_status', {'error': err_msg}, room=sid, namespace='/gui-socket')
            return
        payload['user_context'] = {'id': user_id, 'tier': 'architect'}
...
def on_request_datasets_list(data):
    ...
    try:
        ...
        if not eng_id or not eng_sid:
            err_msg = f'Engine {target_engine_id or "active"} is not connected to the Gateway.'
            if not user_id:
                err_msg = 'Cannot fetch datasets. Please log in to connect to an engine.'
            sio.emit('response_datasets_list', {'error': err_msg}, room=sid, namespace='/gui-socket')
            return
        payload['user_context'] = {'id': user_id, 'tier': 'architect'}
...
def on_request_local_models(data):
    ...
    try:
        ...
        if not eng_id or not eng_sid:
            err_msg = f'Engine {target_engine_id or "active"} is not connected to the Gateway.'
            if not user_id:
                err_msg = 'Cannot fetch models. Please log in to connect to an engine.'
            sio.emit('response_local_models', {'error': err_msg}, room=sid, namespace='/gui-socket')
            return
        payload['user_context'] = {'id': user_id, 'tier': 'architect'}
...
def on_request_training_job_status(data):
    ...
    try:
        ...
        if not eng_id or not eng_sid:
            err_msg = f'Engine {target_engine_id or "active"} is not connected to the Gateway.'
            if not user_id:
                err_msg = 'Cannot fetch job status. Please log in to connect to an engine.'
            sio.emit('response_training_job_status', {'error': err_msg}, room=sid, namespace='/gui-socket')
            return
        payload['user_context'] = {'id': user_id, 'tier': 'architect'}
...
def on_gui_join_session(data):
    ...
    try:
        ...
        if agent_session.user_id != user_id:
            app.logger.warning(f"[AuthZ] DENIED: User {user_id} (SID {sid}) tried to join session {session_id} owned by {agent_session.user_id}.")
            return {'ok': False, 'error': 'Permission denied'}

        if not check_password_hash(agent_session.ws_token_hash, ws_token):
...
def on_core_request_swarm_task(data):
    """
    (R6) Handles a task request from an Originating Engine (A)
    (R6) meant for a Target Engine (B).
    (R6) Data structure: { "target_engine_id": "...", "task_payload": {...} }
    """
    app = current_app._get_current_object()
...
def on_core_swarm_task_result(data):
    """
    (R6) Handles the *result* of a swarm task from a Target Engine (B)
    (R6) and routes it back to the Originating Engine (A).
    (R6) Data structure: { "task_id": "...", "result": {...} }
    """
    app = current_app._get_current_object()
