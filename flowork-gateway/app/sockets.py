...
# File NAME : C:\ORGINAL\flowork-gateway\app\sockets.py total lines 2019 
########################################################################
...
def _resolve_target_engine_sid(session, user_id, target_engine_id):
    ...
    if target_engine_id == 'local-engine-default':
        active_map = globals_instance.engine_manager.active_engine_sessions
        if active_map:
            real_engine_id = list(active_map.keys())[0]
            real_sid = active_map[real_engine_id]
            app.logger.info(f"[_resolve] Auto-mapped 'local-engine-default' -> {real_engine_id} ({real_sid})")
            return real_engine_id, real_sid
        else:
            app.logger.warning("[_resolve] Failed to auto-map 'local-engine-default': No live engines connected.")
