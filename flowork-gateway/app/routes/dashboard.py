...
# File NAME : C:\ORGINAL\flowork-gateway\app\routes\dashboard.py total lines 141 
########################################################################
...
def _get_live_stats_from_core(user_id, app):
    ...
    if active_session and active_session.internal_url and active_session.engine:
        ...
        if not core_server_url:
             ...
             app.logger.warning(f"[Gateway Dashboard] No active engine found. Using Smart Fallback: {core_server_url}")
    ...
    try:
        ...
        return {
            "active_jobs": [],
            "system_overview": {"status": "error", "message": f"Connection Failed: {str(e)}"},
