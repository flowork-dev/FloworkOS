...
# File NAME : C:\ORGINAL\flowork-gateway\app\routes\proxy.py total lines 529 
########################################################################
...
def get_default_core_url():
    """
    Determines the correct Core URL based on the environment.
    """
    if os.environ.get('FLOWORK_CORE_URL'):
        return os.environ.get('FLOWORK_CORE_URL').rstrip('/')
...
def _resolve_target_engine_url(user, target_engine_id):
    ...
    try:
        ...
        if is_owner or is_shared:
            fallback_url = get_default_core_url()
            current_app.logger.warning(f"[Proxy] ID {target_engine_id} not in RAM. Using Smart Fallback -> {fallback_url}")
...
def _proxy_generic_request(subpath):
    ...
    try:
        ...
        if not core_server_url:
            ...
            if not core_server_url:
                 core_server_url = get_default_core_url()
                 current_app.logger.info(f"[Proxy] No engine context found. Defaulting to smart local core: {core_server_url}")
        ...
        if "upload" in subpath or "training" in subpath:
            ...
            print(f"Incoming Content-Type: {request.headers.get('Content-Type', 'MISSING')}", flush=True)
        ...
        resp = requests.request(
            ...
            params=request.args,
            timeout=300 # [FIX] Timeout digedein biar Cloudflare ga putus duluan
        )
        ...
        current_app.logger.error(f"[Proxy] Connection Failed to {core_server_url}: {str(e)}")
        return jsonify({"error": "Failed to reach Core Engine.", "details": str(e), "hint": "Check if Core is running on port 8989"}), 503
    except Exception as e:
...
@handle_cors_options # [FIX] Pake decorator baru
...
def proxy_widgets_detail(subpath):
    return _proxy_generic_request(f"widgets/{subpath}")
...
def proxy_datasets_list():
    """Proxy untuk list dan create datasets"""
...
def proxy_datasets_detail(subpath):
    """Proxy untuk detail dataset dan manipulasi data baris"""
...
def proxy_training(subpath):
    """Proxy untuk start training, cek status job, dan DELETE job"""
...
def proxy_models_detail(subpath):
    """Proxy models detail, termasuk /models/local dan /models/convert"""
...
def proxy_ai_models_all():
    """Proxy fallback untuk list semua model"""
...
def proxy_ai_sub(subpath):
    """
    Proxy generik untuk namespace /ai/
    Menangani:
    - /ai/models (List model gabungan provider & local)
    - /ai/playground (Chat & Streaming)
    """
...
@handle_cors_options
@crypto_auth_required # <-- Ini kuncinya! Auth pakai User Signature, bukan Gateway Token
