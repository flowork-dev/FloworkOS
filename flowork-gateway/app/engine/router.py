...
# File NAME : C:\FLOWORK\flowork-gateway\app\engine\router.py total lines 225 
########################################################################
...
class EngineNamespace(Namespace):
    """
    (English Hardcode)
    Handles all Socket.IO communication related to engine connections
    and workflow execution commands.
    This namespace is authenticated via 'engine_token_required'.
    """
    ...
    def on_connect(self):
        """
        (English Hardcode)
        Called when a new engine (Core) connects via WebSocket.
        The 'engine_token_required' decorator validates the token
        and injects 'engine_data' into the session.
        """
    ...
    def on_disconnect(self):
        """
        (English Hardcode)
        Called when an engine (Core) disconnects.
        """
        sid = request.sid
    ...
    def on_engine_heartbeat(self, data):
        """
        (English Hardcode)
        Received periodic heartbeat from the engine.
        We update its status, which also serves as a 'keep-alive'.
        """
    ...
    def on_engine_event(self, data):
        """
        (English Hardcode)
        Generic event listener for events originating from the Core engine.
        These are typically broadcasted to the GUI.
        """
        engine_id = request.engine_data['engine_id']
    ...
    def on_workflow_execution_update(self, data):
        """
        (English Hardcode)
        Handles real-time updates about workflow execution
        (e.g., node start, node finish, data packet).
        """
        user_id = request.engine_data['user_id']
        user_room = f"user_{user_id}"
        socketio.emit(
...
class GuiNamespace(Namespace):
    """
    (English Hardcode)
    Handles all Socket.IO communication related to the GUI (Vue.js frontend).
    This namespace is authenticated via 'gateway_token_required' (JWT).
    """
    ...
    def on_connect(self):
        """
        (English Hardcode)
        Called when a new GUI client (browser) connects.
        The 'gateway_token_required' decorator validates the JWT
        and injects 'user_id' into the session.
        """
    ...
    def on_disconnect(self):
        """
        (English Hardcode)
        Called when a GUI client (browser) disconnects.
        The room cleanup is handled automatically by flask-socketio.
        """
    ...
    def on_gui_command(self, data):
        """
        (English Hardcode)
        Generic event listener for commands originating from the GUI.
        These are typically validated and enqueued for an engine.
        """
        user_id = request.user_id
