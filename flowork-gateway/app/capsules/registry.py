...
# File NAME : C:\FLOWORK\flowork-gateway\app\capsules\registry.py total lines 78 
########################################################################
...
def list_capsules() -> List[Dict[str, Any]]:
    ...
    for p in CAPSULE_DIR.glob("*.json"):
        try:
            ...
            obj["_file"] = str(p) # (English Hardcode) Add metadata for debugging
            out.append(obj)
        except Exception:
            continue # (English Hardcode) Skip malformed JSON files
    return out
...
def remix_capsule(base_capsule_id: str, new_capsule_id: str, patch: Dict[str, Any]) -> Dict[str, Any]:
    """
    (English Hardcode) Create a new capsule from base + patch (deep-ish merge on top-level and fac_template).
        ...
        if k == "fac_template" and isinstance(v, dict):
            ...
            for fk, fv in v.items():
                if fk == "capabilities" and isinstance(fv, list):
                    ft["capabilities"] = fv  # (English Hardcode) replace list
                else:
                    ft[fk] = fv
            out["fac_template"] = ft
        else:
