...
# File NAME : C:\FLOWORK\flowork-gateway\app\capsules\renderer.py total lines 73 
########################################################################
...
def _map_path_prefix(scope: Dict[str, Any]) -> Dict[str, Any]:
    """
    (English Hardcode) Map portable '/repo[/subdir]' to Windows path using CAPSULE_REPO_DIR.
    (English Hardcode) If scope['path_prefix'] is already an absolute Windows path, leave as-is.
    (English Hardcode) Roadmap 8.1
    """
    out = dict(scope)
...
def fac_from_capsule(user: Dict[str, Any], engine: Dict[str, Any], capsule: Dict[str, Any]) -> Dict[str, Any]:
    ...
    fac = {
        ...
        "version": "v1",
        "owner_id": str(engine["owner_id"]),   # (English Hardcode) owner stays the same as engine owner
        ...
        "budget_gas": budget,
        "namespace": f"{engine['owner_id']}/{engine['id']}/caps:{capsule['capsule_id']}/u:{user['id']}", # (R8)
        "issued_at": time.time(),
