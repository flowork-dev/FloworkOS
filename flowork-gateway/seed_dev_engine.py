...
# File NAME : C:\FLOWORK\flowork-gateway\seed_dev_engine.py total lines 134 
########################################################################
...
def seed_default_engine(db, User, RegisteredEngine):
    """
    [MUTATED BY FOWORK DEV]
    Fungsi ini memastikan Engine Default terdaftar tanpa menyebabkan IntegrityError pada session.
    """

    print("--- Flowork Gateway Development Engine Seeder (Strong Mutation) ---")
    ...
    try:
        ...
        if not user:
            ...
            try:
                ...
                new_user = User(
                    ...
                    email=f"auto_owner_{public_address[:8]}@flowork.local",
        ...
        try:
            ...
            if ghost_engines:
                from app.models import UserEngineSession # Import dinamis untuk menghindari circular

                for ghost in ghost_engines:
                    print(f"[INFO] Cleaning up ghost engine & sessions: {ghost.id}")

                    UserEngineSession.query.filter_by(engine_id=ghost.id).delete()


                    db.session.delete(ghost)

                db.session.commit()
                print(f"[SUCCESS] Ghost engines and orphaned sessions purged.")
        except Exception as e:
            print(f"[WARN] Error cleaning ghost engines safely: {e}")
            db.session.rollback()
