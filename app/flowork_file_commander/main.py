...
# File NAME : C:\FLOWORK\app\flowork_file_commander\main.py total lines 225
########################################################################
...
# [SOP Phase 3] Reconfigure conduits for real-time muscle log streaming
try:
...
def resolve_path(path_str, user_id):
    ...
    # Handle system-to-user redirection logic
    if "/users/system" in clean_path:
        clean_path = clean_path.replace("/users/system", f"/users/{real_id}")

    # [Rule Emas] Anti-Nesting: Cek jika path sudah diawali UID
    ...
    # Convert relative path to absolute sandbox path
...
def action_delete_item(params, user_id):
    ...
    if not target.exists():
        # [Rule Emas] Menjadikan silent success untuk stabilitas batch
...
def action_delete_batch(params, user_id):
    ...
    for p in paths:
        ...
        try:
            # [Rule Emas] Defensive check for race conditions during batch
            if target.exists():
                if target.is_dir(): shutil.rmtree(target)
                else: os.remove(target)
                deleted += 1
            # Skip silently if already gone
...
if __name__ == "__main__":
    ...
    try:
        ...
        # [SOP Phase 5] Output result to Core with forced flush
