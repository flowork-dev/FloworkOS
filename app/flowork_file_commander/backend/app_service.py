...
# File NAME : C:\FLOWORK\app\flowork_file_commander\backend\app_service.py total lines 289
########################################################################
...
class FileManagerService(BaseAppService):
    ...
    Now upgraded to support Phase 3 (Muscle Bridge).
    """
            ...
            if data_root.exists():
                for entry in os.scandir(data_root):
                    if entry.is_dir() and entry.name != "system":
                        return entry.name # Nemu! Balikin ID user asli (ac26...)
        except: pass
        return "system" # Nyerah, balik ke system
    ...
    async def _delegate_to_muscle(self, action, payload, user_id="system"):
        ...
        try:
            ...
            self.logger.warning(f"⚠️ [Bridge] Muscle delegation failed for {action}: {e}. Falling back to Resident.")
            return None
    ...
    async def delete_batch_items(self, paths: list, user_id: str):
        ...
        log_msg = f"Batch Delete: {deleted_count} items removed."
