...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\swarm.py total lines 259 
########################################################################
...
class SwarmCoordinator:
    """
    (R6) Coordinating fan-out requests to multiple engines and aggregating results.
    - send_task_fn: user-provided dispatcher (can use your existing WS/Gateway)
    - max_workers: thread pool size (defaults to len(engine_ids) per fan_out if None)
    - default_timeout_s: per-engine timeout
    - retries/backoff: simple exponential backoff (sleep inside worker thread)
    - timeline: optional TimelineLogger (from kernel.timeline) for audit trail
    """
    def __init__(self,
    ...
    def fan_out(self,
        ...
        """
        (R6) Broadcast the same task to multiple engines and aggregate results.
        (R6) Returns a structured dict containing per-engine results and a summary.
        """
    ...
    def majority(self, engine_ids: List[str], task: Dict[str, Any], **kw) -> Dict[str, Any]:
        """(R6) Shorthand for quorum='majority'."""
    ...
    def map_reduce(self,
                   ...
                   *,
                   ...
                   backoff_base_s: Optional[float] = None) -> Dict[str, Any]:
        """
        (R6) Build per-engine tasks (map) and reduce successful results to an aggregate.
        (R6) reducer receives list of (engine_id, value) for ok==True engines.
        """
        swarm_id = str(uuid.uuid4())
    ...
    def _call_with_retry(self,
        ...
        """
        (R6) Execute send_task_fn with simple retry & backoff.
        (R6) Returns (ok, value, error_str, latency_ms).
        """
        attempts = 0
...
class LocalSwarmRegistry:
    """
    (R6) Minimal local bus for multi-agent on the same node.
    - register(engine_id, handler): handler(task_dict) -> dict (result)
    - make_send_task_fn(): returns a SendTaskFn compatible with SwarmCoordinator
    """
    def __init__(self):
