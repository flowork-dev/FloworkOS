...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\episodic.py total lines 186 
########################################################################
...
class EpisodicStore:
    """
    (Refactor R4)
    Multi-tenant episodic storage, backward-compatible dengan R3 BIOS.
    - KV (R3): <base>/episodic/<namespace>/<key>.json (menyimpan full dict)
    - Episodes (R4): <base>/episodes/<namespace>/<episode_id>.json
    """
    ...
    def _kv_file_for(self, key: str) -> Path:
        """(R4 Refactor) Gets the full, safe file path for a given KV key."""
    ...
    def write(self, key: str, data: dict):
        """
        (R3-Compatible) Writes a full dictionary to the KV store.
        (R4-Upgrade) Uses atomic write.
        """
    ...
    def read(self, key: str) -> dict | None:
        """(R3-Compatible) Reads a full dictionary from the KV store."""
        file_path = self._kv_file_for(key)
        try:
    ...
    def delete(self, key: str) -> bool:
        """(R3-Compatible) Deletes a key from the KV store."""
        file_path = self._kv_file_for(key)
        try:
            os.remove(file_path)
            return True
    ...
    def save_episode(self, payload: Dict[str, Any]) -> str:
        """
        (R4) Save an episode JSON under episodes/<namespace>/<episode_id>.json.
        If 'episode_id' absent, a UUID4 is generated.
        """
    ...
    def load_episode(self, episode_id: str) -> Dict[str, Any]:
        """(R4) Loads a specific episode JSON by its ID."""
        file_path = self.episodes_root / f"{episode_id}.json"
    ...
    def list_episodes(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        (R4) Return recent episodes (by file mtime desc).
        Useful for GUI quick listing.
        """
