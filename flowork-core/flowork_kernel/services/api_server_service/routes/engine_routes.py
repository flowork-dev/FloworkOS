...
class EngineRoutes(BaseApiRoute):
    ...
    async def handle_get_live_stats(self, request):
        ...
        self.logger(f"[Core Engine API] Received /live-stats request for User-ID: {user_id_from_header}", "INFO", "ApiServer")
        active_jobs = []
        ...
        if self.service_instance.job_statuses:
            ...
            for job_id, job_data in all_jobs_copy:
                ...
                if end_time and end_time >= twenty_four_hours_ago:
                    preset_name = job_data.get("preset_name", "Unknown Preset")
        ...
        self.logger(f"[Core Engine API] Found {len(active_jobs)} active jobs relevant to user {user_id_from_header}", "DEBUG", "ApiServer")
        preset_count = 0
        if self.service_instance.preset_manager and user_id_from_header:
            try:
                ...
                preset_count = len(preset_list)
                 ...
                 self.logger(f"[Core Engine API] Error getting preset count for user {user_id_from_header}: {e}", "WARN", "ApiServer")
                 preset_count = 0
        system_overview = {
        ...
        live_data = {
            ...
            "top_slowest_presets": final_slowest_list,
