...
class ApiServerService(BaseService):
    ...
    def _load_api_routes(self):
        ...
        all_route_classes = [FilesystemRoutes, EngineRoutes, PresetRoutes]
            ...
            try:
                ...
                for route, handler in registered_routes.items():
                    ...
                    if not asyncio.iscoroutinefunction(handler):
                        self.kernel.write_to_log(
                            f"    - WARNING: Handler for {method} {pattern} is not async!", "WARN"
                        )
                        self.app.router.add_route(method, pattern, handler)
                    else:
                        self.app.router.add_route(method, pattern, handler)
                    self.kernel.write_to_log(
                        f"    - Registered: {method} {pattern}", "DETAIL"
                    )
            except Exception as e:
                import traceback
                self.kernel.write_to_log(
        ...
        self.kernel.write_to_log("    - Registered: GET /ops/advice", "DETAIL")
        self.kernel.write_to_log("    - Registered: POST /webhook/{preset_name}", "DETAIL")
    ...
    async def middleware_handler(self, request, handler):
        ...
        with self.tracer.start_as_current_span(span_name, context=trace_context) as span:
            ...
            headers = {
                ...
                "Access-Control-Allow-Headers": "X-API-Key, Content-Type, Authorization, X-Flowork-User-ID, X-Flowork-Engine-ID, X-Signature, X-User-Address, X-Signed-Message, traceparent",
            }
    ...
    def _authenticate_request(self, request):
        ...
        if not expected_key:
            self.kernel.write_to_log(
                "GATEWAY_SECRET_TOKEN not set. Skipping internal API authentication check.", "WARN"
            )
            return True
