...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\agent_executor_service\agent_executor_service.py total lines 646 
########################################################################
...
class AgentExecutorService(BaseService):
    ...
    async def execute_remote_swarm_task(self, task_payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        (R6) This is the "worker" function.
        (R6) It is called by GatewayConnector when *another* engine
        (R6) sends *this* engine a task.
        """
        try:
    ...
    def _find_tool_manifest_as_node(self, component_id: str) -> Dict[str, Any] | None:
        """
        (R6) Helper for remote swarm worker. Finds a tool manifest
        (R6) from any manager and fakes a "Node" structure for the executor.
        """
        manifest = None
    ...
    def _build_agent_tool_list(self, connected_tools: list) -> list:
        """(R6) Gathers all tool manifests AND adds the new swarm_fan_out tool."""
        tools_for_agent = []
    ...
    def _get_tools_prompt_from_manifests(self, tools_for_agent: list) -> str:
        """(R6) Generates a string listing available tools from the *full* tool list."""
        tools_list = []
    ...
    def _register_tools_for_swarm(self, swarm_registry: LocalSwarmRegistry, tools_list: list, base_payload: dict, workflow_context_id: str):
        """(R6) Dynamically creates handlers for each tool and registers them with the local swarm."""
        ...
        for node_data in tools_list:
            ...
            def create_tool_handler(node_info: dict, tool_name: str) -> Callable[[Dict[str, Any]], Dict[str, Any]]:
                ...
                def _tool_handler(task_payload: Dict[str, Any]) -> Dict[str, Any]:
                    """(R6) This is the actual function called by SwarmCoordinator.send_task_fn."""
