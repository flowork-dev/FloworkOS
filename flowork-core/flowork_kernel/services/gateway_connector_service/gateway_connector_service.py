...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\gateway_connector_service\gateway_connector_service.py total lines 1417 
########################################################################
...
class GatewayConnectorService(BaseService):
    ...
    async def send_gateway_swarm_task(self, target_engine_id: str, task_payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        (R6) This is the main dispatcher function (SendTaskFn) for the *Gateway* SwarmCoordinator.
        (R6) It sends a task to the Gateway and waits (asynchronously) for the result.
        """
    ...
    async def emit_to_gateway(self, event_name: str, payload: dict):
        ...
        try:
            ...
            if event_name != 'core:agent_token' and not event_name.startswith('core:request_swarm_task'):
    ...
    async def forward_event_to_gateway(self, event_name, subscriber_id, payload):
        """
        (English Hardcode) (GEMINI ADDED & FIXED) Forwards an event from the Core's internal event_bus
        (English Hardcode) to the main Socket.IO gateway to be sent to the GUI.
        (English Hardcode) NOTE: This callback receives 3 arguments from the event bus.
        """
        try:
            self.logger.info(f"[GatewayConnector] DEBUG: forward_event_to_gateway CALLED with event: {event_name}")
            ...
            if event_name in GUI_SAFE_EVENTS:
                self.logger.info(f"[GatewayConnector] Forwarding event '{event_name}' to GUI.")
    ...
    def register_event_handlers(self):
        ...
        async def execute_workflow(data):
            ...
            (English Hardcode) (MODIFIED - R5) This now selects a strategy using the Router.
            """
            try:
                if not isinstance(data, dict) or data.get('v') != CURRENT_PAYLOAD_VERSION:
                ...
                try:
                    ...
                    if edges_to_insert:
                        cursor.executemany(
                            "INSERT INTO Edges (workflow_id, source_node_id, target_node_id) VALUES (?, ?, ?)",
                            edges_to_insert
                        )
                        self.logger.info(f"Successfully inserted {len(edges_to_insert)} edge definitions.")
        ...
        async def on_stop_workflow(data):
            """ (English Hardcode) Handles the 'stop_workflow' event from the Gateway. """
            try:
        ...
        async def on_pause_workflow(data):
            """ (English Hardcode) Handles the 'pause_workflow' event from the Gateway. """
            try:
        ...
        async def save_preset(data):
            ...
            try:
                ...
                user_context = real_data.get('user_context', {})
                user_id = user_context.get('id')
                preset_name = real_data.get('preset_name')
                preset_data = real_data.get('preset_data')
                ...
                self.logger.error(f"Error handling 'save_preset': {e}", exc_info=True)
        ...
        async def gw_start_session(data):
            """
            (English Hardcode) Implements contract: Gateway -> Core (start)
            (English Hardcode) data = { "type":"start", "session_id", "engine_id", "intent", "context":{...}, "fac":{...} }
            (English Hardcode) R7 MODIFIED - This handler now validates the FAC.
            """
        ...
        async def gw_cancel_session(data):
            """
            (English Hardcode) Implements contract: Gateway -> Core (cancel)
            (English Hardcode) data = { "type":"cancel", "session_id" }
            """
            session_id = data.get('session_id')
        ...
        async def gw_agent_input(data):
            """
            (English Hardcode) Implements contract: Gateway -> Core (input)
            (English Hardcode) data = { "type":"input", "session_id", "payload":{...} }
            """
            session_id = data.get('session_id')
        ...
        async def gateway_execute_swarm_task(data):
            """
            (R6) This engine is being TOLD to execute a task by the Gateway.
            (R6) This is the "worker" side of a multi-node swarm.
            """
        ...
        async def gateway_swarm_task_result(data):
            """
            (R6) This is the *result* coming back from the Gateway for a task we sent out.
            """
