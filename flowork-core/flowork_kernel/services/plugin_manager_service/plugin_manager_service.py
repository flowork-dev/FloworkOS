...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\plugin_manager_service\plugin_manager_service.py total lines 436 
########################################################################
...
class PluginManagerService(BaseService):
    ...
    def get_instance(self, plugin_id):
        ...
        if not plugin_data.get("is_installed", False):
            self.logger.error(f"Attempted to get instance for non-installed plugin: {plugin_id}")
            return None
    ...
    def _worker_install_dependencies(self, plugin_id: str, on_complete: callable):
        ...
        try:
            ...
            if not os.path.exists(requirements_path):
                self.logger.info(f"No requirements.txt found for '{plugin_id}'. Marking as installed.")
                with open(install_marker_path, "w") as f:
                    f.write("installed")
                self.loaded_plugins[plugin_id]["is_installed"] = True
                on_complete(plugin_id, True, "No requirements.txt found, marked as installed.")
                return

            self.logger.info(f"Creating venv for '{plugin_id}' at {venv_path}...")
            ...
            self.logger.info(f"Installing requirements for '{plugin_id}'...")
            ...
            self.logger.info(f"Installation successful for '{plugin_id}'. Creating marker file.")
            with open(install_marker_path, "w") as f:
                f.write("installed")
            self.loaded_plugins[plugin_id]["is_installed"] = True
            on_complete(plugin_id, True, f"Dependencies installed successfully: {result.stdout}")
            ...
            try:
                ...
                self.logger.error(f"Failed to cleanup failed install for '{plugin_id}': {cleanup_e}")
    ...
    def _worker_uninstall_dependencies(self, plugin_id: str, on_complete: callable):
        ...
        try:
            ...
            if os.path.exists(install_marker_path):
                os.remove(install_marker_path)
                self.logger.info(f"Removed install marker for '{plugin_id}'.")
            if os.path.isdir(venv_path):
                shutil.rmtree(venv_path)
