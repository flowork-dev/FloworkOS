...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\logging_service\logging_service.py total lines 147 
########################################################################
...
class LoggingService(BaseService):
    """
    Manages all logging operations within the Flowork kernel.
    It supports logging to console, file, and (conditionally) to the GUI.
    """
    ...
    def start(self):
        """
        Start the service and safely acquire the GatewayConnector.
        We fetch the gateway_connector here to avoid circular dependencies
        during the initial kernel boot process. (English Comment)
        """
        try:
    ...
    def setup(self, log_level_str='DEBUG', log_to_file=True):
        """
        Configures the logger (level, format, handlers).
        """
