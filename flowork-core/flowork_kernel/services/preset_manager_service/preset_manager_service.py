...
class PresetManagerService(BaseService):
    ...
    def __init__(self, kernel, service_id: str):
        ...
        self.logger.debug(
            "Service 'PresetManager' (Flow-Chain & SQL-Populator) initialized."
        )
    def start(self):
        """
        ...
        if not self.db_service:
            self.logger.critical("CRITICAL: PresetManager failed to get DatabaseService from Singleton.")
        os.makedirs(self.users_data_path, exist_ok=True)
        self.logger.info("PresetManagerService started and dependencies loaded.")
    ...
    def _sync_trigger_rules_for_preset(
        ...
        if not self.state_manager or not self.trigger_manager:
            self.logger.warning(
                "Cannot sync trigger rules: StateManager or TriggerManager not available."
            )
            return
        ...
        for rule_id in rules_to_delete:
            ...
            self.logger.info(
                f"Removed old trigger rule '{rule_id}' for preset '{preset_name}'."
            )
        if not is_delete and workflow_data:
            ...
            for node in trigger_nodes:
                ...
                new_rule = {
                    "name": f"Trigger for {preset_name} ({node['name']})",
                ...
                self.logger.info(
                    f"Created/Updated trigger rule '{rule_id}' for preset '{preset_name}'."
                )
    ...
    def get_preset_list(self, user_id: str):
        ...
        try:
            ...
            self.logger.error(f"Could not get preset list for user '{user_id}': {e}")
            return []
    def get_preset_data(self, name: str, user_id: str):
        ...
        if not is_valid:
            self.logger.error(f"CRITICAL: Integrity check failed for preset '{name}'. {message}")
            return None
        latest_file_path, _ = self._get_latest_version_file(workflow_path)
        if not latest_file_path:
            self.logger.warning(f"No version files found for preset '{name}', but folder exists.")
            return None
        try:
            with open(latest_file_path, "r", encoding="utf-8") as f:
                chain_data = json.load(f)
                return chain_data.get("workflow_data")
        except (IOError, json.JSONDecodeError) as e:
            self.logger.error(
                f"Could not read or parse latest preset file '{latest_file_path}': {e}"
            )
            return None
    def save_preset(self, name: str, workflow_data: dict, user_id: str, signature: str) -> bool:
        ...
        try:
            with self._save_lock:
                ...
                self.logger.info(f"Flow-Chain: Saved version {new_version} for preset '{name}'.")
                if not self.db_service:
                    raise Exception("DatabaseService is not available.")
                conn = self.db_service.create_connection()
                if not conn:
                     raise Exception("Failed to create DB connection for SQL population.")
                try:
                    cursor = conn.cursor()
                    self.logger.info(f"SQL POPULATOR: Refreshing SQL tables for workflow_id: {workflow_id_sql}")
                    ...
                    conn.commit()
                    self.logger.info(f"SQL POPULATOR: Successfully synced {len(nodes_to_insert)} nodes and {len(edges_to_insert)} edges.")
                except Exception as sql_e:
                    conn.rollback()
                    self.logger.critical(f"SQL POPULATOR: FAILED. Rolling back changes. Error: {sql_e}")
                    raise sql_e
                finally:
                    conn.close()
            ...
            self.logger.error(f"Could not save preset '{name}': {e}", exc_info=True)
            return False
        ...
        try:
            with self._save_lock:
                if os.path.isdir(workflow_path):
                    ...
                    self.logger.info(f"Flow-Chain: Deleted preset folder '{name}'.")
                else:
                    self.logger.warning(f"Flow-Chain: Preset folder '{name}' not found, skipping delete.")
                if not self.db_service:
                    raise Exception("DatabaseService is not available.")
                conn = self.db_service.create_connection()
                if not conn:
                     raise Exception("Failed to create DB connection for SQL cleanup.")
                try:
                    cursor = conn.cursor()
                    self.logger.info(f"SQL CLEANUP: Deleting records for workflow_id: {workflow_id_sql}")
                    cursor.execute("DELETE FROM Nodes WHERE workflow_id = ?", (workflow_id_sql,))
                    cursor.execute("DELETE FROM Edges WHERE workflow_id = ?", (workflow_id_sql,))
                    cursor.execute("DELETE FROM Workflows WHERE workflow_id = ?", (workflow_id_sql,))
                    conn.commit()
                    self.logger.info(f"SQL CLEANUP: Successfully deleted workflow records.")
                except Exception as sql_e:
                    conn.rollback()
                    self.logger.critical(f"SQL CLEANUP: FAILED. Rolling back changes. Error: {sql_e}")
                    raise sql_e
                finally:
                    conn.close()
                self._sync_trigger_rules_for_preset(
            ...
            self.logger.error(f"Could not delete preset '{name}': {e}")
            return False
    ...
    def load_preset_version(self, name: str, version_filename: str, user_id: str):
        ...
        is_valid, message = verify_workflow_chain(workflow_path)
        if not is_valid:
            self.logger.error(f"CRITICAL: Integrity check failed for preset '{name}' when trying to load version. {message}")
            return None
        try:
            ...
            self.logger.error(
                f"Could not load preset version '{version_filename}': {e}"
            )
            return None
    def delete_preset_version(
        self, name: str, version_filename: str, user_id: str
    ) -> bool:
        self.logger.warning("Deleting a single version is not allowed in Flow-Chain model as it breaks integrity.")
        return False
