...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\ai_training_service\ai_training_service.py total lines 230 
########################################################################
...
class AITrainingService(BaseService):
    ...
    def start_fine_tuning_job(
        ...
        model_info = self.ai_manager.local_models.get(base_model_id)

        if not model_info:
            model_info = self.ai_manager.local_models.get(f"(Local Model) {base_model_id}")

        if not model_info:
            model_info = self.ai_manager.local_models.get(f"(Local) {base_model_id}")

        if not model_info or not model_info.get("full_path"):
            available_keys = list(self.ai_manager.local_models.keys())
            self.logger.error(f"Model lookup failed. Requested: '{base_model_id}'. Available: {available_keys}")
            return {
                "error": f"Base model '{base_model_id}' could not be found. Please ensure the model is downloaded and visible in AI Models."
            }
        ...
        self.training_jobs[job_id] = {
            ...
            "job_id": job_id # Ensure ID is in the object
        }
    ...
    def _training_worker(
        ...
        try:
            with open(temp_log_path, "w", encoding="utf-8") as log_file:
                ...
                os.environ["HF_HOME"] = hf_cache_dir # Newer versions use HF_HOME
