...
# File NAME : C:\ORGINAL\flowork-core\flowork_kernel\services\startup_service\startup_service.py total lines 267 
########################################################################
...
class StartupService(BaseService):
    ...
    def _auto_install_dependencies(self):
        ...
        for i in range(max_retries):
            try:
                if os.path.exists(modules_path) and len(os.listdir(modules_path)) > 0:
                    self.logger.info(f"✅ [Auto-Installer] Volume mounted. Found items in modules.")
                    break
            except: pass
        ...
        if install_count > 0:
            ...
            self.logger.info("⚡ [Auto-Installer] System up-to-date. No new dependencies found.")
