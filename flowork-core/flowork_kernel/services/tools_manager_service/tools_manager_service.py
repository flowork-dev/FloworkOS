...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\tools_manager_service\tools_manager_service.py total lines 362 
########################################################################
...
class ToolsManagerService(BaseService):
    ...
    def get_instance(self, tool_id):
        ...
        if not tool_data.get("is_installed", False):
            self.logger.error(f"Attempted to get instance for non-installed tool: {tool_id}")
            return None
    ...
    def _worker_install_dependencies(self, tool_id: str, on_complete: callable):
        ...
        try:
            ...
            if not os.path.exists(requirements_path):
                self.logger.info(f"No requirements.txt found for '{tool_id}'. Marking as installed.")
                with open(install_marker_path, "w") as f:
                    f.write("installed")
                self.loaded_tools[tool_id]["is_installed"] = True
                on_complete(tool_id, True, "No requirements.txt found, marked as installed.")
                return
            ...
            self.logger.info(f"Installing requirements for '{tool_id}'...")
            ...
            self.logger.info(f"Installation successful for '{tool_id}'. Creating marker file.")
            with open(install_marker_path, "w") as f:
                f.write("installed")
            self.loaded_tools[tool_id]["is_installed"] = True
            on_complete(tool_id, True, f"Dependencies installed successfully: {result.stdout}")
            ...
            try:
                ...
                self.logger.error(f"Failed to cleanup failed install for '{tool_id}': {cleanup_e}")
    ...
    def _worker_uninstall_dependencies(self, tool_id: str, on_complete: callable):
        ...
        try:
            ...
            if os.path.exists(install_marker_path):
                os.remove(install_marker_path)
                self.logger.info(f"Removed install marker for '{tool_id}'.")
            if os.path.isdir(venv_path):
                shutil.rmtree(venv_path)
