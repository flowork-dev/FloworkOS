...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\module_manager_service\module_manager_service.py total lines 514 
########################################################################
...
class ModuleManagerService(BaseService):
        ...
        try:
            if not os.path.exists(self.modules_dir):
                ...
                if alt_dir and os.path.exists(alt_dir):
                    self.logger.warning(f"[Compat] Using fallback modules path: {alt_dir}")
    ...
    def get_instance(self, module_id):
        ...
        if module_data.get("is_paused", False):
            self.logger.warning(f"Attempted to get instance for paused module: {module_id}")
            return None

        if not module_data.get("is_installed", False):
            self.logger.error(f"Attempted to get instance for non-installed module: {module_id}")
            return None
        ...
        is_path_added = False
        venv_path = os.path.join(module_data["path"], ".venv")
        ...
        try:
            ...
            if site_packages_path and os.path.isdir(site_packages_path):
                if site_packages_path not in sys.path:
                    ...
                    self.logger.debug(f"Added venv path to sys.path for '{module_id}': {site_packages_path}")
            ...
            if is_venv_path_added:
                try:
                    ...
                    self.logger.debug(f"Removed venv path from sys.path for '{module_id}'")
                except ValueError:
                    pass
    ...
    def _worker_install_dependencies(self, module_id: str, on_complete: callable):
        ...
        try:
            if module_id not in self.loaded_modules:
                raise FileNotFoundError(f"Module '{module_id}' not found in loaded_modules.")
            ...
            if not os.path.exists(requirements_path):
                self.logger.info(f"No requirements.txt found for '{module_id}'. Marking as installed.")
                with open(install_marker_path, "w") as f:
                    f.write("installed")
                self.loaded_modules[module_id]["is_installed"] = True
                on_complete(module_id, True, "No requirements.txt found, marked as installed.")
                return

            self.logger.info(f"Creating venv for '{module_id}' at {venv_path}...")
            ...
            self.logger.info(f"Installing requirements for '{module_id}'...")
            ...
            self.logger.info(f"Installation successful for '{module_id}'. Creating marker file.")
            with open(install_marker_path, "w") as f:
                f.write("installed")
            self.loaded_modules[module_id]["is_installed"] = True
            on_complete(module_id, True, f"Dependencies installed successfully: {result.stdout}")

        except Exception as e:
            self.logger.error(f"Failed to install dependencies for '{module_id}': {e}")
            self.logger.debug(traceback.format_exc())
            on_complete(module_id, False, f"Installation failed: {e}")
            try:
                ...
                self.logger.error(f"Failed to cleanup failed install for '{module_id}': {cleanup_e}")

    def install_component_dependencies(self, module_id: str, on_complete: callable):

        self.logger.info(f"Queuing dependency installation for module: {module_id}")
    ...
    def _worker_uninstall_dependencies(self, module_id: str, on_complete: callable):
        ...
        try:
            ...
            if os.path.exists(install_marker_path):
                os.remove(install_marker_path)
                self.logger.info(f"Removed install marker for '{module_id}'.")
            if os.path.isdir(venv_path):
                shutil.rmtree(venv_path)
                self.logger.info(f"Removed venv directory for '{module_id}'.")

            self.loaded_modules[module_id]["is_installed"] = False
            on_complete(module_id, True, "Component dependencies uninstalled successfully.")

        except Exception as e:
            self.logger.error(f"Failed to uninstall dependencies for '{module_id}': {e}")
            self.logger.debug(traceback.format_exc())
            on_complete(module_id, False, f"Uninstallation failed: {e}")

    def uninstall_component_dependencies(self, module_id: str, on_complete: callable):

        self.logger.info(f"Queuing dependency uninstallation for module: {module_id}")
