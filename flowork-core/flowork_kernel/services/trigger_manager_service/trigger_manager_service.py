...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\services\trigger_manager_service\trigger_manager_service.py total lines 478 
########################################################################
...
class TriggerManagerService(BaseService):
    ...
    def _worker_install_dependencies(self, trigger_id: str, on_complete: callable):
        ...
        try:
            ...
            if not os.path.exists(requirements_path):
                self.logger.info(f"No requirements.txt found for '{trigger_id}'. Marking as installed.")
                with open(install_marker_path, "w") as f:
                    f.write("installed")
                self.loaded_triggers[trigger_id]["is_installed"] = True
                on_complete(trigger_id, True, "No requirements.txt found, marked as installed.")
                return
            self.logger.info(f"Creating venv for '{trigger_id}' at {venv_path}...")
            ...
            self.logger.info(f"Installing requirements for '{trigger_id}'...")
            ...
            self.logger.info(f"Installation successful for '{trigger_id}'. Creating marker file.")
            with open(install_marker_path, "w") as f:
                f.write("installed")
            self.loaded_triggers[trigger_id]["is_installed"] = True
            on_complete(trigger_id, True, f"Dependencies installed successfully: {result.stdout}")
        except Exception as e:
            ...
            try:
                ...
                self.logger.error(f"Failed to cleanup failed install for '{trigger_id}': {cleanup_e}")
    ...
    def _worker_uninstall_dependencies(self, trigger_id: str, on_complete: callable):
        ...
        try:
            ...
            if os.path.exists(install_marker_path):
                os.remove(install_marker_path)
                self.logger.info(f"Removed install marker for '{trigger_id}'.")
            if os.path.isdir(venv_path):
                shutil.rmtree(venv_path)
