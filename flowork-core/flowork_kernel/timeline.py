...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\timeline.py total lines 133 
########################################################################
...
class TimelineLogger:
    """
    (Refactor R4)
    Durable + fast tail, dibangun di atas R3 Logger.
    - (R3) Pakai RotatingFileHandler untuk audit JSONL.
    - (R4) Path di-scope ke namespace: <base>/timeline/<namespace>/timeline.log
    - (R4) In-memory tail (deque) untuk GUI export_summary() cepat.
    """
    ...
    def log(self, event_type: str, data: Dict[str, Any]):
        """
        (R4 REFACTOR)
        Logs event ke disk (R3) DAN in-memory tail (R4).
        """
        try:
    ...
    def export_summary(self) -> Dict[str, Any]:
        """
        (R4 BARU)
        Minimal summary for GUI/Gateway: returns only the recent tail.
        """
        return {
    ...
    def close(self):
        """(R3 DIPERTAHANKAN) Closes all handlers for this logger."""
