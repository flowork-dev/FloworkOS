...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\capabilities\agent_run.py total lines 45 
########################################################################
...
class AgentRun:
    """
    (English Hardcode) Meta-capability that permits running a specific engine_id.
    """
    def __init__(self, scope: Dict[str, str]):
        """
        (English Hardcode) scope: {"engine_id": "<id>" or "*"}
        """
    ...
    def permit_engine(self, engine_id_to_run: str) -> None:
        """
        (English Hardcode) Runtime check (called from Core when starting a run).
        (English Hardcode) Checks if the FAC-scoped engine_id matches the requested one.
        """
        if self.engine_id == "*":
            return
