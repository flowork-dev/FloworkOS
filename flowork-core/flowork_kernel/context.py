...
# File NAME : C:\FLOWORK\flowork-core\flowork_kernel\context.py total lines 325 
########################################################################
...
class AgentContext:
    """
    (Refactor R4)
    'Pawang' Agent. Sekarang juga memegang FacEnforcer.
    """
    ...
    def _enforce_permission(self, p_type: str, p_target: str, p_name_for_log: str):
        """(R4 BARU) Helper untuk validasi izin."""
        if self.kill_flag:
    ...
    def fs_read(self, file_path: str) -> str:
        """(Refactor R4) Cek izin DULU, baru gas."""
    ...
    def fs_write(self, file_path: str, content: str):
        """(R4 BARU) Aksi baru: Tulis file (fs_write)."""
...
def boot_agent(agent_id: str, fac_data: dict) -> AgentContext:
    """
    (Refactor R4)
    BIOS Bootstrapper. Sekarang juga load FacEnforcer.
    """
    try:
