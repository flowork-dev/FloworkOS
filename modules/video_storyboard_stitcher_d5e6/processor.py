...
class VideoStoryboardStitcherModule(BaseModule, IExecutable):
    ...
    def _run_ffmpeg_stitch(
        ...
        if music_path and os.path.exists(music_path):
            status_updater("Adding background music...", "INFO")
            command.extend(
                ["-i", music_path]
            )
            if original_audio_mode == "merge":
                self.logger("Audio mode: Merge", "DEBUG")
                ...
                self.logger("Audio mode: Replace", "DEBUG")
        ...
        try:
            ...
            self.logger(f"FFmpeg STDOUT: {result.stdout}", "DEBUG")
            self.logger(f"FFmpeg STDERR: {result.stderr}", "DEBUG")
