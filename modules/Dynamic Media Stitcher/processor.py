...
# File NAME : C:\FLOWORK\modules\Dynamic Media Stitcher\processor.py total lines 545 
########################################################################
...
class DynamicMediaStitcherModule(BaseModule, IExecutable, IDataPreviewer):
    ...
    def _run_ffmpeg_stitch_video_only(self, clip_list, output_path, status_updater):
        ...
        try:
            ...
            status_updater("Stitching video clips...", "INFO")
            subprocess.run(
                command,
                check=True,
                capture_output=True,
    ...
    def _run_ffmpeg_stitch_audio_only(self, clip_list, output_path, status_updater):
        ...
        try:
            ...
            status_updater("Stitching audio clips...", "INFO")
            subprocess.run(
                command,
                check=True,
                capture_output=True,
