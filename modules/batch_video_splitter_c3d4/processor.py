...
# File NAME : C:\ORGINAL\modules\batch_video_splitter_c3d4\processor.py total lines 185 
########################################################################
        ...
        for pair in folder_pairs:
            ...
            for i, filename in enumerate(files):
                ...
                cmd = [
                    self.ffmpeg_path,
                    "-i", input_path,
                    "-c:v", "libx264",                    # Codec video standar tapi kuat
                    "-preset", "ultrafast",               # TURBO MODE: Prioritas Speed No. 1
                    "-crf", "23",                         # Kualitas balance (makin kecil makin bagus tapi size gede)
                    "-c:a", "aac",                        # Audio encode
                    "-sc_threshold", "0",                 # Matiin deteksi scene biar keyframe patuh sama durasi
                    "-force_key_frames", f"expr:gte(t,n_forced*{segment_duration})", # Paksa bikin 'guntingan' tiap X detik
                    "-map", "0",
