...
# File NAME : C:\FLOWORK\modules\batch_video_splitter_c3d4\processor.py total lines 195 
########################################################################
...
class BatchVideoSplitterModule(BaseModule, IExecutable, IDataPreviewer):
    ...
    def execute(
        ...
        if not self.ffmpeg_path:
            error_msg = (
                "FFmpeg is not available. This module cannot function."
            )
        ...
        if not folder_pairs:
            error_msg = (
                "No source/output folder pairs have been configured."
            )
            if "data" not in payload:
                payload["data"] = {}
            payload["data"]["error"] = error_msg
        ...
        try:
            ...
            error_msg = "Segment duration must be a valid integer."
            if "data" not in payload:
                payload["data"] = {}
            payload["data"]["error"] = error_msg
            return {"payload": payload, "output_name": "error"}
